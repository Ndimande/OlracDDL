...
class RetainedInfoScreen extends StatefulWidget {
  final int retainedCatchID;
  final int indexID;

  const RetainedInfoScreen({this.retainedCatchID, this.indexID});

  @override
  _RetainedInfoScreenState createState() => _RetainedInfoScreenState();
}

class _RetainedInfoScreenState extends State<RetainedInfoScreen> {
  ...
  Widget dataRow(String variableHeader, String info) {
    return Padding(
  ...
  Widget _dataTable() {
    final details = Column(children: [
      dataRow('Common Name', _retainedCatch.species.commonName),
      dataRow('Species', _retainedCatch.species.scientificName),
      dataRow('Green Weight', (_retainedCatch.greenWeight / 1000).toString()),
      dataRow('No. of Individuals', _retainedCatch.individuals.toString()),
      dataRow('Uploaded', _trip.isUploaded ? 'Yes' : 'No'),
    ]);
